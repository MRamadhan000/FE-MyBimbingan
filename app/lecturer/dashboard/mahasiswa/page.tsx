...
import { 
  FiSearch, FiFilter, FiBell, FiArrowRight, 
  FiMoreVertical, FiClock, FiCheckCircle, FiAlertCircle 
} from "react-icons/fi";
    ...
    <div className="max-w-6xl mx-auto space-y-8 py-8 px-4">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div className="space-y-1">
          <h1 className="text-3xl font-black text-slate-900 tracking-tight">Daftar Mahasiswa</h1>
          <p className="text-slate-500 font-medium text-sm">Kelola dan pantau seluruh mahasiswa bimbingan Anda.</p>
        </div>
          ...
          <table className="w-full border-collapse text-left">
            <thead>
              <tr className="bg-slate-50/50 border-b border-slate-100">
                <th className="p-6 text-[10px] font-black uppercase text-slate-400 tracking-[0.2em]">Mahasiswa</th>
                <th className="p-6 text-[10px] font-black uppercase text-slate-400 tracking-[0.2em]">Status Terakhir</th>
                <th className="p-6 text-[10px] font-black uppercase text-slate-400 tracking-[0.2em]">Progres</th>
                <th className="p-6 text-[10px] font-black uppercase text-slate-400 tracking-[0.2em]">Update</th>
                <th className="p-6 text-center"></th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-50 text-sm">
              {MAHASISWA_LIST.map((mhs) => (
                <tr key={mhs.id} className="hover:bg-slate-50/50 transition-colors group">
                  <td className="p-6">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <p className="font-black text-slate-900 leading-none">{mhs.nama}</p>
                        <p className="text-[10px] font-bold text-slate-400 mt-1 uppercase tracking-widest">{mhs.nim}</p>
                      </div>
                    </div>
                  </td>
                  <td className="p-6">
                    <span className={`inline-flex items-center gap-2 px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest border ${
                      mhs.status === "REVISI" ? "bg-rose-50 border-rose-100 text-rose-600" :
                      mhs.status === "MENUNGGU REVIEW" ? "bg-blue-50 border-blue-100 text-blue-600" :
                  ...
                  <td className="p-6">
                    <div className="flex flex-col">
                      <span className={`font-black ${mhs.lastUpdate > 7 ? "text-rose-500" : "text-slate-900"}`}>
                        {mhs.lastUpdate} Hari Lalu
                      </span>
                      {mhs.lastUpdate > 7 && (
                        <span className="text-[9px] font-bold text-rose-400 uppercase tracking-tighter italic">Warning: Pasif</span>
                      )}
                    </div>
                  </td>
                  <td className="p-6">
      ...
      <div className="p-6 bg-slate-50 border border-slate-200 rounded-[2rem] flex flex-wrap gap-8 justify-center md:justify-start">
        <div className="flex items-center gap-3">
          ...
          <p className="text-[10px] font-black text-slate-500 uppercase">1 Menunggu Review</p>
        </div>
        <div className="flex items-center gap-3">
          <div className="w-2 h-2 rounded-full bg-rose-500"></div>
          <p className="text-[10px] font-black text-slate-500 uppercase">1 Perlu Revisi</p>
        </div>
        <div className="flex items-center gap-3">
          <div className="w-2 h-2 rounded-full bg-emerald-500"></div>
          <p className="text-[10px] font-black text-slate-500 uppercase">1 Sudah Disetujui</p>
        </div>
      </div>
